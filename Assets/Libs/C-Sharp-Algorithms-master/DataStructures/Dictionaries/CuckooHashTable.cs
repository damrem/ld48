...
namespace DataStructures.Dictionaries
{
    ...
    public class CuckooHashTable<TKey, TValue> where TKey : IComparable<TKey>
    {
        ...
        /// </summary>
        private class CHashEntry<TEntryKey, TEntryValue> where TEntryKey : IComparable<TEntryKey>
        {
            public TEntryKey Key { get; set; }
            public TEntryValue Value { get; set; }
            public bool IsActive { get; set; }

            public CHashEntry() : this(default(TEntryKey), default(TEntryValue), false) { }

            public CHashEntry(TEntryKey key, TEntryValue value, bool isActive)
            {
                Key = key;
                Value = value;
                IsActive = isActive;
