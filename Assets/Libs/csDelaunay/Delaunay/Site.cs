...
namespace csDelaunay {
    ...
    public class Site : ICoord {
        ...
        public static Site Create(Vector2f p, int index, float weight) {
            if(pool.Count > 0) {
                return pool.Dequeue().Init(p, index, weight);
            }
            else {
        ...
        public List<Site> NeighborSites() {
            //Dbg.Log(this, "region", Region(new Rectf(-5, -5, 10, 10)));
        ...
        public List<Vector2f> Region(Rectf clippingBounds) {
            ...
            if(edgeOrientations == null || region == null) {
                ReorderEdges();
